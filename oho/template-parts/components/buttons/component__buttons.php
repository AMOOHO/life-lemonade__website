...
<?php if (!empty($btns) && is_array($btns)) : ?>
  <div class="block--buttons block--<?= $rowIndex; ?><?= $componentIndex ?? '' ?>">
    ...
    ?>
      ...
      <?php if ($type === 'email'): ?>
        <a class="mail-link" <?= secure_mail_link_attr($email); ?>>
          <span class="button p"><?= $emailAsCaptionToggle ? antispambot($email) : esc_html($caption); ?></span>
        </a>

      <?php elseif ($type === 'tel'): ?>
        <a href="tel:<?= format_phone_nr($tel, true, ''); ?>">
          <span class="button p"><?= $telAsCaptionToggle ? format_phone_nr($tel) : esc_html($caption); ?></span>
        </a>

      <?php elseif ($type === 'internal-link'): ?>
        <a href="<?= esc_url($url); ?>" target="_self">
          <span class="button p <?= esc_attr($type); ?>"><?= esc_html($caption); ?></span>
        </a>
